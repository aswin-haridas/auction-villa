...
export default function Bidding() {
  ...
  const now = new Date();
  const timeRemaining = auctionData?.endTime - now.getTime();
  ...
  return (
    <div>
      <h1 className={`${anton.className} font-anton text-2xl `}>Auction</h1>
      <div className="p-4 border border-red-800 flex flex-col md:flex-row h-screen">
        ...
        <div className="flex-1 space-y-4 mt-4 md:mt-0 md:ml-4">
          <h1 className="text-3xl">{auctionData.name}</h1>
          <h1 className="text-green-500">Starting Price: {auctionData.price}u</h1>
          <h1>End Time: {timeRemaining}</h1>
